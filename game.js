document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadAssets() {
        const imageSources = {
            ...
            downloadIcon: 'https://i.imgur.com/2V3ERsH.png',
            captchaLogo: 'https://i.imgur.com/4gHk2kN.png',
        };
    ...
    function handleClick() {
        for (let i = state.entities.length - 1; i >= 0; i--) {
            ...
            if (entity.onClick && entity.onClick(state.mouse)) { return; }
        }
    }
    function spawnAd() {
    ...
    class PopupAd extends Entity {
        constructor(x, y) { super(x, y, 380, 200); this.clickDamage = 10; this.points = 20; this.hasFake = state.difficulty > 1.3 && Math.random() < 0.4; this.buttons = {real: { x: 0, y: 0, w: 20, h: 20 }, fake: this.hasFake ? { x: 0, y: 0, w: 20, h: 20 } : null }; }
        update() { super.update(); this.buttons.real.x = this.x + this.w - 30; this.buttons.real.y = this.y + 10; if (this.hasFake) { this.buttons.fake.x = this.x + this.w - 60; this.buttons.fake.y = this.y + 10; } }
        ...
        draw() {
            ctx.save();
            const scaledW = this.w * this.scale; const scaledH = this.h * this.scale;
            const scaledX = this.x + (this.w - scaledW) / 2; const scaledY = this.y + (this.h - scaledH) / 2;
            ctx.globalAlpha = this.opacity;
            ctx.shadowColor="rgba(0,0,0,0.5)"; ctx.shadowBlur=30 * this.scale;
            ctx.fillStyle="#1E1E24"; drawRoundRect(scaledX, scaledY, scaledW, scaledH, 8); ctx.fill();
            ctx.shadowBlur=0;
            if (this.scale > 0.9) {
                ...
                const t=(t,e)=>{t.hover=isPointInRect(state.mouse,t);ctx.fillStyle=t.hover?e?"#2ECC40":"#FF4136":"#555";state.mouse.down&&t.hover&&(ctx.fillStyle="#fff");drawRoundRect(t.x,t.y,t.w,t.h,4);ctx.fill();ctx.strokeStyle="#EFEFEF";ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(t.x+5,t.y+5);ctx.lineTo(t.x+15,t.y+15);ctx.moveTo(t.x+15,t.y+5);ctx.lineTo(t.x+5,t.y+15);ctx.stroke()};
                t(this.buttons.real,!0); this.hasFake&&t(this.buttons.fake,!1);
            }
            ctx.restore();
        }
    }
    class VideoAd extends Entity {
        constructor(x, y) { super(x, y, 420, 236); this.clickDamage = 15; this.points = 30; this.progress = 0; this.closeButton = { x: 0, y: 0, w: 80, h: 30, text: "Skip" }; this.thumb = Math.random() < 0.5 ? 'videoThumb1' : 'videoThumb2';}
        update() { super.update(); if (this.progress < 100) this.progress += 0.2 * state.difficulty; this.closeButton.x = this.x + this.w - 100; this.closeButton.y = this.y + this.h - 45; }
        ...
        draw() {
            ...
            const scaledX = this.x + (this.w - scaledW) / 2; const scaledY = this.y + (this.h - scaledH) / 2;
            ctx.globalAlpha = this.opacity;
            const thumbImg=assets.images[this.thumb];
            ctx.fillStyle="#000"; drawRoundRect(scaledX, scaledY, scaledW, scaledH, 8); ctx.fill();
            if (this.scale > 0.9) {
                if(thumbImg){ctx.globalAlpha=this.opacity*.8;ctx.drawImage(thumbImg,this.x,this.y,this.w,this.h);ctx.globalAlpha=this.opacity}
                ctx.fillStyle="rgba(0,0,0,0.4)"; ctx.fillRect(this.x,this.y,this.w,this.h);
                if(this.progress<100){ctx.fillStyle="#fff";ctx.globalAlpha=this.opacity*0.5;ctx.beginPath();ctx.moveTo(this.x+this.w/2-20,this.y+this.h/2-30);ctx.lineTo(this.x+this.w/2-20,this.y+this.h/2+30);ctx.lineTo(this.x+this.w/2+30,this.y+this.h/2);ctx.closePath();ctx.fill();ctx.globalAlpha=this.opacity}
                else{this.closeButton.hover=isPointInRect(state.mouse,this.closeButton);ctx.fillStyle=this.closeButton.hover?"rgba(255,255,255,0.3)":"rgba(255,255,255,0.1)";drawRoundRect(this.closeButton.x,this.closeButton.y,this.closeButton.w,this.closeButton.h,5);ctx.fill();ctx.fillStyle="#fff";ctx.font="bold 14px Poppins";ctx.textAlign="center";ctx.fillText(this.closeButton.text,this.closeButton.x+40,this.closeButton.y+21)}
    ...
    class CookieBanner extends Entity { // This class does not use scaling, so its logic was already correct.
    ...
    class OSNotification extends Entity { // This class also does not use scaling.
    ...
    class NewsletterAd extends Entity {
        constructor(x, y) { super(x, y, 400, 220); this.clickDamage=15;this.points=35;this.subscribeBtn={x:0,y:0,w:300,h:45};this.closeLink={text:"No, thanks.",rect:{x:0,y:0,w:0,h:0}}}
        update() { super.update(); this.subscribeBtn.x = this.x + 50; this.subscribeBtn.y = this.y + 130; const textWidth=100; this.closeLink.rect={x:this.x+(this.w-textWidth)/2,y:this.y+185,w:textWidth,h:20}; }
        ...
        draw() {
            ...
            const scaledX = this.x + (this.w - scaledW) / 2; const scaledY = this.y + (this.h - scaledH) / 2;
            ctx.globalAlpha = this.opacity;
            ctx.fillStyle='#fff'; drawRoundRect(scaledX, scaledY, scaledW, scaledH, 12); ctx.fill();
            if (this.scale > 0.9) {
                ctx.fillStyle='#111';ctx.font="bold 24px Poppins";ctx.textAlign='center';ctx.fillText("Don't Miss Out!",this.x+this.w/2,this.y+50);
                ctx.font="14px Roboto";ctx.fillText("Sign up for our newsletter for exclusive deals!",this.x+this.w/2,this.y+80);
                this.subscribeBtn.hover=isPointInRect(state.mouse,this.subscribeBtn);ctx.fillStyle=this.subscribeBtn.hover?'#FF4136':'#dd2c00';
                drawRoundRect(this.subscribeBtn.x,this.subscribeBtn.y,this.subscribeBtn.w,this.subscribeBtn.h,8);ctx.fill();
                ctx.fillStyle='#fff';ctx.font="bold 16px Poppins";ctx.fillText("SUBSCRIBE NOW",this.x+this.w/2,this.y+160);
                ctx.font="14px Roboto";const textWidth=ctx.measureText(this.closeLink.text).width; this.closeLink.rect.w = textWidth; this.closeLink.rect.x = this.x + (this.w - textWidth) / 2;
                this.closeLink.hover=isPointInRect(state.mouse,this.closeLink.rect);ctx.fillStyle=this.closeLink.hover?'#007bff':'#888';
                ctx.fillText(this.closeLink.text,this.x+this.w/2,this.y+195);
            }
            ctx.restore();
        }
    }
    class Powerup extends Entity {
        ...
        draw(){
            ctx.save();
            const scaledSize = this.w * this.scale;
            const scaledX = this.x + (this.w - scaledSize) / 2;
            const scaledY = this.y + (this.h - scaledSize) / 2;
            const t="#2ECC40"; ctx.shadowColor=t; ctx.shadowBlur=30*this.scale;
            ctx.fillStyle=t; drawRoundRect(scaledX, scaledY, scaledSize, scaledSize, scaledSize/2); ctx.fill();
            if (this.scale > 0.9) {
                ctx.shadowBlur=0; ctx.strokeStyle="#fff"; ctx.lineWidth=3;
                ctx.font="bold 30px Poppins"; ctx.textAlign='center';
                ctx.strokeText("+",this.x+this.w/2,this.y+this.h/2+11);
            }
            ctx.restore();
        }
    }
    ...
    class FakeDownloadAd extends Entity {
        constructor(x, y) { super(x, y, 420, 250); this.clickDamage = 20; this.points = 50; this.fakeButtons = [ { x: 0, y: 0, w: 340, h: 50, text: "DOWNLOAD NOW" }, { x: 0, y: 0, w: 180, h: 30, text: "Start Download" } ]; this.realLink = { text: "No thanks, I don't want this file.", rect: {} }; }
        update() { super.update(); this.fakeButtons[0].x = this.x + 40; this.fakeButtons[0].y = this.y + 120; this.fakeButtons[1].x = this.x + 120; this.fakeButtons[1].y = this.y + 180; const textWidth = 180; this.realLink.rect = { x: this.x + (this.w - textWidth) / 2, y: this.y + this.h - 25, w: textWidth, h: 15 }; }
        onClick(mouse) { if (!this.alive || this.scale < 1 || !isPointInRect(mouse, this)) return false; if (isPointInRect(mouse, this.realLink.rect)) { this.destroy(this.points); return true; } for (const btn of this.fakeButtons) { if (isPointInRect(mouse, btn)) { takeDamage(this.clickDamage); playSound(assets.sounds.fakeClick); return true; } } takeDamage(this.clickDamage / 2); return true; }
        draw() {
            ctx.save();
            const scaledW = this.w * this.scale; const scaledH = this.h * this.scale;
            const scaledX = this.x + (this.w - scaledW) / 2; const scaledY = this.y + (this.h - scaledH) / 2;
            ctx.globalAlpha = this.opacity;
            ctx.fillStyle = '#F0F2F5'; drawRoundRect(scaledX, scaledY, scaledW, scaledH, 10); ctx.fill();
            if (this.scale > 0.9) {
                const icon = assets.images.downloadIcon; if (icon) ctx.drawImage(icon, this.x + 30, this.y + 30, 64, 64);
                ctx.fillStyle = '#111'; ctx.font = "bold 20px Poppins"; ctx.textAlign = 'left'; ctx.fillText("Your download is ready!", this.x + 110, this.y + 65);
                this.fakeButtons.forEach((btn, i) => { btn.hover = isPointInRect(state.mouse, btn); ctx.fillStyle = btn.hover ? '#21a34a' : '#28a745'; drawRoundRect(btn.x, btn.y, btn.w, btn.h, 6); ctx.fill(); ctx.fillStyle = '#fff'; ctx.font = `bold ${i === 0 ? 18 : 14}px Poppins`; ctx.textAlign = 'center'; ctx.fillText(btn.text, btn.x + btn.w / 2, btn.y + btn.h / 2 + 6); });
                ctx.font = "12px Roboto";
                const textWidth = ctx.measureText(this.realLink.text).width; this.realLink.rect.w = textWidth; this.realLink.rect.x = this.x + (this.w-textWidth)/2;
                this.realLink.hover = isPointInRect(state.mouse, this.realLink.rect); ctx.fillStyle = this.realLink.hover ? '#007bff' : '#aaa';
                ctx.textAlign='center'; ctx.fillText(this.realLink.text, this.x + this.w / 2, this.y + this.h - 15);
            }
            ctx.restore();
        }
    }
    class CaptchaAd extends Entity {
        constructor(x, y) { super(x, y, 300, 130); this.clickDamage = 10; this.points = 40; this.checkbox = { x: 0, y: 0, w: 30, h: 30 }; }
        update() { super.update(); this.checkbox.x = this.x + 20; this.checkbox.y = this.y + 55; }
        onClick(mouse) { if (!this.alive || this.scale < 1 || !isPointInRect(mouse, this)) return false; if (isPointInRect(mouse, this.checkbox)) { this.destroy(this.points); return true; } takeDamage(this.clickDamage); return true; }
        draw() {
            ctx.save();
            const scaledW = this.w * this.scale; const scaledH = this.h * this.scale;
            const scaledX = this.x + (this.w - scaledW) / 2; const scaledY = this.y + (this.h - scaledH) / 2;
            ctx.globalAlpha = this.opacity;
            ctx.fillStyle = '#f9f9f9'; drawRoundRect(scaledX, scaledY, scaledW, scaledH, 4); ctx.fill();
            if (this.scale > 0.9) {
                ...
                if (this.checkbox.hover && state.mouse.down) { ctx.fillStyle = '#007bff'; ctx.beginPath(); ctx.moveTo(this.checkbox.x+8, this.checkbox.y+15); ctx.lineTo(this.checkbox.x+13, this.checkbox.y+20); ctx.lineTo(this.checkbox.x+22, this.checkbox.y+10); ctx.stroke(); }
                ctx.font = "16px Roboto"; ctx.fillStyle = '#111';
                ctx.fillText("I'm not a robot", this.checkbox.x + 40, this.y + 78);
                const logo = assets.images.captchaLogo; if (logo) ctx.drawImage(logo, this.x + this.w - 70, this.y + 50, 60, 60);
            }
            ctx.restore();
        }
    }
