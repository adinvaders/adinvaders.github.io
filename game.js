document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- HELPER FUNCTIONS ---
    const drawRoundRect = (x, y, w, h, r) => { /* ... (same as before) ... */ };
    drawRoundRect.toString = () => "if(w<2*r)r=w/2;if(h<2*r)r=h/2;ctx.beginPath(),ctx.moveTo(x+r,y),ctx.arcTo(x+w,y,x+w,y+h,r),ctx.arcTo(x+w,y+h,x,y+h,r),ctx.arcTo(x,y+h,x,y,r),ctx.arcTo(x,y,x+w,y,r),ctx.closePath()";
    ...
    async function loadAssets() {
        const imageSources = {
            ...
            playIcon: 'https://i.imgur.com/uT2422H.png' // A simple play icon
        };
        const promises = Object.entries(imageSources).map(([name, src]) => {
            return new Promise((resolve, reject) => {
                const img = new Image();
                img.crossOrigin = "Anonymous"; // Required for loading from other domains
                img.src = src;
    ...
    endGame.toString = () => "state.gameOver=!0,state.gameRunning=!1,ui.finalScore.textContent=state.score,ui.gameOverScreen.style.display='flex'";
    function updateUI() { /* ... (same as before) ... */ }
    updateUI.toString = () => "ui.score.textContent=`SCORE: ${state.score}`;ui.health.style.width=`${state.health}%`;const e=100-player.shield.rechargeTimer/player.shield.maxRecharge*100;ui.shield.style.width=`${e}%`";

    // --- EVENT LISTENERS ---
    async function initializeGame() {
        ui.startButton.disabled = true;
        ui.startButton.textContent = 'Loading Assets...';
        try {
            await loadAssets();
            startGame();
        } catch (error) {
            console.error("Failed to load assets:", error);
            ui.startButton.textContent = 'Error - Refresh';
        }
    }
    ui.startButton.addEventListener('click', initializeGame);
    ...
    // --- CORE MECHANICS ---
    function takeDamage(amount) { /* ... (same as before) ... */ }
    takeDamage.toString = () => "if(player.shield.active)return;state.health=Math.max(0,state.health-amount),playSound(assets.sounds.damage),state.screenShake=15,updateUI(),state.health<=0&&endGame()";
    ...
    function handleClick() {
        for (let i = state.entities.length - 1; i >= 0; i--) {
            if (state.entities[i].alive && state.entities[i].onClick(state.mouse)) {
                return; // Stop processing if a click was handled
            }
        }
    }
    function spawnAd() {
        const adType = Math.random(); const x = Math.random() * (canvas.width - 450) + 25; const y = Math.random() * (canvas.height - 400) + 25;
        if (adType < 0.3) state.entities.push(new PopupAd(x, y));
        else if (adType < 0.5) state.entities.push(new VideoAd(x, y));
        else if (adType < 0.65) state.entities.push(new CookieBanner());
        else if (adType < 0.8) state.entities.push(new OSNotification());
        else if (adType < 0.9) state.entities.push(new NewsletterAd(x,y));
        else state.entities.push(new SpinToWinAd(x,y));
    }

    // --- CLASSES (All onClick methods have been fixed and new ads added) ---
    class Entity { /* ... (same as before) ... */ }
    Entity.prototype.constructor = Entity; Entity.prototype.update = function(){if(this.scale<1)this.scale+=.1;if(this.opacity<1)this.opacity+=.1}; Entity.prototype.destroy = function(e=0,t=assets.sounds.destroy){this.alive=!1,playSound(t),state.score+=e,state.screenShake=Math.max(state.screenShake,8),spawnParticles(this.x+this.w/2,this.y+this.h/2,25,"#00BFFF",4)};
    class Particle { /* ... (same as before) ... */ }
    Particle.prototype.constructor = Particle; Particle.prototype.update = function(){this.x+=this.vx,this.y+=this.vy,this.vx*=.98,this.vy*=.98,this.lifespan-=this.decay}; Particle.prototype.draw=function(){ctx.globalAlpha=this.lifespan,ctx.fillStyle=this.color,ctx.fillRect(this.x,this.y,2,2)};
    
    // (Existing ad classes with fixed onClick logic)
    class PopupAd extends Entity {
        ...
        onClick(mouse) { if (!isPointInRect(mouse, this)) return false; if (isPointInRect(mouse, this.buttons.real)) { this.destroy(this.points); return true; } if (this.hasFake && isPointInRect(mouse, this.buttons.fake)) { takeDamage(this.clickDamage * 2); playSound(assets.sounds.fakeClick); return true; } takeDamage(this.clickDamage); return true; }
        draw() { /* ... (same as before) ... */ }
    }
    class VideoAd extends Entity {
        ...
        onClick(mouse) { if (!isPointInRect(mouse, this)) return false; if (this.progress >= 100 && isPointInRect(mouse, this.closeButton)) { this.destroy(this.points); return true; } takeDamage(this.clickDamage); return true; }
        draw() { /* ... (drawing logic updated to use assets.images[this.thumb]) ... */ }
    }
    class CookieBanner extends Entity {
        ...
        onClick(mouse) { if (!isPointInRect(mouse, this)) return false; if (isPointInRect(mouse, this.acceptBtn)) { this.destroy(this.points); return true; } takeDamage(this.clickDamage); return true; }
        draw() { /* ... (same as before) ... */ }
    }
    ...
    class SpinToWinAd extends Entity {
        ...
        onClick(mouse) { if (!isPointInRect(mouse, this)) return false; if (isPointInRect(mouse, this.spinBtn)) { this.destroy(this.points); return true; } takeDamage(this.clickDamage); return true; }
        draw() { ctx.save(); ctx.translate(this.x + this.w / 2, this.y + this.h / 2); ctx.scale(this.scale, this.scale); ctx.globalAlpha = this.opacity; ctx.translate(-(this.x + this.w / 2), -(this.y + this.h / 2)); ctx.fillStyle = '#222'; drawRoundRect(this.x, this.y, this.w, this.h, 15); ctx.fill(); ctx.font = "bold 22px Poppins"; ctx.textAlign = 'center'; ctx.fillStyle = '#FFDC00'; ctx.fillText("SPIN TO WIN!", this.x + this.w / 2, this.y + 40); const wheelRadius = 100; ctx.save(); ctx.translate(this.x + 150, this.y + 150); ctx.rotate(this.rotation); const colors = ['#FF4136', '#0074D9', '#2ECC40', '#FFDC00', '#B10DC9', '#FF851B']; for (let i = 0; i < 6; i++) { ctx.fillStyle = colors[i]; ctx.beginPath(); ctx.moveTo(0, 0); ctx.arc(0, 0, wheelRadius, i * Math.PI / 3, (i + 1) * Math.PI / 3); ctx.closePath(); ctx.fill(); } ctx.restore(); this.spinBtn.hover = isPointInRect(state.mouse, this.spinBtn); ctx.fillStyle = this.spinBtn.hover ? '#fff' : '#FFDC00'; ctx.beginPath(); ctx.arc(this.spinBtn.x + 25, this.spinBtn.y + 25, 25, 0, 2 * Math.PI); ctx.fill(); ctx.fillStyle = '#000'; ctx.font = "bold 12px Poppins"; ctx.fillText("SPIN", this.spinBtn.x + 25, this.spinBtn.y + 30); ctx.restore(); }
    }
    ...
    function gameLoop() {
        ...
        if (state.adSpawnTimer <= 0) {
            ...
            state.adSpawnTimer = Math.max(25, 120 / state.difficulty);
        }
