...
const AdManager = {
    adDefinitions: {
        ...
        gremlinAd: { threat: 20, points: 250, create: () => AdManager.createAd('gremlin-ad', {
            title: 'Security Warning', clickDamage: 10,
    ...
    createAd(className, options) {
        ...
        const headerHTML = `<div class="ad-header"><span>${options.title}</span><button class="close-btn">×</button></div>`;
        const contentHTML = `<div class="ad-content">${options.content || ''}</div>`;
        ad.element.innerHTML = `${options.closeable !== false ? headerHTML : `<div class="ad-header"><span>${options.title}</span></div>`}${contentHTML}`;

        const closeBtn = ad.element.querySelector('.close-btn');
        if (closeBtn) {
            const definition = this.adDefinitions[className.split(' ')[0]];
            closeBtn.onclick = e => { e.stopPropagation(); this.destroyAd(ad, definition.points); };
        }
